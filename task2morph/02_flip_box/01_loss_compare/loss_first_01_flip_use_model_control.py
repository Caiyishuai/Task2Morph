...
if __name__ == '__main__':
    ...
    if model_name == "choose_good":
        net.load_state_dict(torch.load("../model/flip_model_parameter_reference_1.pkl"))

    print(net)

    for seedi in range(20):
        args.seed = seedi
        random.seed ( seedi )
        ...
        modify_flip_box_size(args.model,size_a,size_b,size_c)
        ...
        env_data_x = [box_x,box_y,size_a,size_b,size_c]
        env_data_x = torch.tensor(env_data_x,dtype=torch.double) # dtype=torch.double
        env_data_x = Variable(env_data_x)
        cage_params = net(env_data_x).detach().numpy()  # use NN output
        ...
        def loss_and_grad(params):
            with torch.no_grad():
                f, info = forward(params, backward_flag = True)
                print_info('iteration ', len(f_log), ', num_sim = ', num_sim, ', Objective = ', f, info)
                # smoth
                if f_log!=[] and f-f_log[-1][-1] > 200:
                    f_log.append(np.array([num_sim, f_log[-1][-1]]))
                else:   
        ...
        if not play_mode:
            ...
            res = scipy.optimize.minimize(loss_and_grad, np.copy(params), method = "L-BFGS-B", jac = True, callback = callback_func, bounds = bounds,\
                                          options={'maxiter': 100})
        ...
        import pandas as pd
        columns=['Episodes','Loss']
        loss_data_pd = pd.DataFrame(f_log)
        loss_data_pd.columns=columns
        loss_data_pd.to_csv('result_new/flip__'+str(box_x)+" "+str(box_y)+" "+str(size_a)+" "+str(size_b)+" "+str(size_c)+\
                            "_mod="+str(model_name)+'_control.csv',header=True,index=True)

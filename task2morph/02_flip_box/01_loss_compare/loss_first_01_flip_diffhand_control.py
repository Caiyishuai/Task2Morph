...
if __name__ == '__main__':
    ...
    for seedi in range(20):
        ...
        if not play_mode:
            ...
            res = scipy.optimize.minimize(loss_and_grad, np.copy(params), method = "L-BFGS-B", jac = True, \
                                          callback = callback_func, bounds = bounds, options={'maxiter': 100})
        ...
        print("Final morphology parameters:",params[-cage_num:]) 
        ...
        # loss_data_pd.to_csv('result_new/flip_loss_seed='+str(args.seed)+"_mod=diffhandcontrol"+'.csv',header=True,index=True)
        loss_data_pd.to_csv('result_new/flip__'+str(box_x)+" "+str(box_y)+" "+str(size_a)+" "+str(size_b)+" "+str(size_c)+\
                            '_mod=diffhand_control.csv',header=True,index=True)

'''
description:
DiffHand co-optimization for flip box task
'''
import os
import sys
...
if __name__ == '__main__':
    ...
    # optimize_design_flag = False # Only optimazate control
    ...
    # set up camera
    # sim.viewer_options.camera_pos = np.array([2.8, -4, 1.8]) 
    ...
    if not play_mode:
        ...
        if visualize:
            ...
            callback_func(params, render = True, log = False, record = args.record, record_path = args.record_file_name + "_diffhand_init.gif")
    ...
    if visualize:
        ...
        print_info('Press [Esc] to continue')
        callback_func(params, render = True, record = args.record, record_path = args.record_file_name + "_diffhand_optimized.gif", log = False)
        ...
        loss_data_pd.to_csv('loss_data/flip_loss_seed='+str(random_seed)+"_mod=diffhand"+'.csv',header=True,index=True)

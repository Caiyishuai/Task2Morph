...
if __name__ == '__main__':
    ...
    if model_name == "choose_good":
        net.load_state_dict(torch.load("model/flip_model_parameter_reference_3.pkl"))
    print(net)
    ...
    # set up camera
    sim.viewer_options.camera_pos = np.array([3.5, -4.3, 1.8]) 
    # init design params
    design = Design()
    design_np = Design_np()
    ...
    env_data_x = env_data_x.clone().detach()
    ...
    if visualize:
        ...
        loss_data_pd.to_csv('loss_data/flip_loss_seed='+str(random_seed)+"_mod="+str(model_name)+'.csv',header=True,index=True)

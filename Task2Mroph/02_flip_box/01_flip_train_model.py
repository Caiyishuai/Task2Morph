...
if __name__ == '__main__':
    ...
    while (iters <= args.data_size or bufferpoint <= 80):
        ...
        # change box pos     
        box_x = random.uniform(-25, 25)
        box_y = random.uniform(-5,5)    
        box_x = round(box_x,3)
        box_y = round(box_y,3)
        ...
        env_data_x = torch.tensor([box_x,box_y,size_a,size_b,size_c],dtype=torch.double) 
        ...
        if not optimize_design_flag:
            ...
            params[0:ndof_u * num_ctrl_steps] = action # the first half of the control, the second half of the form
        ...
        # save morphology parameters and f
        tmp_save_ls=[]
        tmp_save_ls.append(box_x)
        tmp_save_ls.append(box_y)
        tmp_save_ls.append(size_a)
        tmp_save_ls.append(size_b)
        tmp_save_ls.append(size_c)
        ...
        f, info = forward(params, backward_flag = False)
        tmp_save_ls.append(info['f_flip'])
    ...
    columns=['box_x','box_y','size_a','size_b','size_c']
    for i in range(cage_num):
        columns.append("design_params_"+str(i))
    columns.append("Objective")
    columns.append("f_flip")
             
    my_save_ls = np.array(my_save_ls)
    my_save_ls = pd.DataFrame(my_save_ls)
    my_save_ls.columns=columns
    my_save_ls.to_csv('data/flip_cage_params_f_'+str(args.data_size)+'.csv',header=True,index=True)
    
    
    # Save data
    torch.save(net.state_dict(), "model/flip_model_parameter"+str(args.data_size)+".pkl")

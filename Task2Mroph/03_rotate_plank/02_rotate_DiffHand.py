'''
description:
DiffHand for rotate plank task
'''
import os
import sys
...
if __name__ == '__main__':
    ...
    box_x = random.uniform(-10, 30)
    box_y = random.uniform(-10,5)    
    box_x = round(box_x,3)
    box_y = round(box_y,3)
    
    
    print("new pos:",box_x, " ",box_y)
    env_test_data = np.array([box_x,box_y])
    new_str_pos = str(env_test_data[0])+" "+str(env_test_data[1])+" -8"
    ...
    # set up camera
    sim.viewer_options.camera_pos = np.array([2.5, -4, 2.3])
